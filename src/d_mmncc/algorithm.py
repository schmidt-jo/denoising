...
from d_mmncc import chambollepock, handlers, options, plotting
import tqdm
...
class Denoizer:
    def __init__(self, config: options.Opts):
        self.single_iteration: bool = config.single_iteration
        ...
        self.num_cp_runs: int = config.num_max_runs   # set number of runs
        self.use_mp: bool = config.mp
        self.mp_headroom: int = config.mp_headroom
        ...
        self.out_path: str = config.output_path
    ...
    def get_nc_stats(self, data: np.ndarray, run: int = 0):
        log_module.info("extract Noise characteristics")
        nc_chi, _, noise_array = handlers.extract_chi_noise_characteristics_from_nii(
            nii_data=data,
        ...
        if self.visualize:
            plotting.plot_noise_histogram(
                noise_array, path=self.out_path, name=f"run-{run}",
                num_channels=self.num_channels, sigma=self.sigma
            )
    ...
    def denoize_nii_data(self, data: np.ndarray, save_plot: str = ""):
        ...
        log_module.info(f"multiprocessing using {num_cpus} cpus")
